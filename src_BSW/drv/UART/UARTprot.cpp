...
#define REMOTEPORT uart2

#pragma section ".com.uart"

//Ringbuffer which will contain the data coming from the Zigbee port
REMOTE_zigbeeBuffer_t UARTPROT_remoteRxBuffer(0x0d,0x0e);
REMOTE_zigbeeBuffer_t UARTPROT_remoteTxBuffer(0x0d,0x0e);
 ...
 * return RC_SUCCESS
 */
RC_t UARTPROT_remote_transmit(const REMOTE_raw_t * const pTxBuffer, const SIG_IDX_TYPE index)
{
	uint8_t data;
 ...
 */
void UARTPROT_remote_transmit_isr(const REMOTE_raw_t pTxBuffer)
{
	uint8_t data;
	if (pTxBuffer->readData(data))
	{
 ...
 */
bool UARTPROT_remote_receive_isr(const REMOTE_raw_t pRxBuffer, bool& fullProtocol)
{
	uint8_t data;
