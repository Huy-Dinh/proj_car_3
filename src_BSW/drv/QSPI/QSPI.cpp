...
void _QSPI_TX_ISR(int module_num)
{
	...
	{
		sQSPI_module_transact(&QSPI_mod[module_num], QSPI_exchange_write);
	}
...
void _QSPI_PHASE_TRANSITION_ISR(int arg)
{
	if (DERIVATE_QSPI_NUM_MOD > arg)
	{
		...
		sQSPI_module_transact(&QSPI_mod[arg], QSPI_exchange_read);
	}
						  ...
						  const QSPI_exchange_param_t *apParam)
{
	...
	{
		pPkt = &pModule->mpPktQ[pModule->mnPktQWR];
