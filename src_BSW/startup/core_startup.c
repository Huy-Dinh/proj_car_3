...
EXTERN _FARDATA	uint32_t			__INT_TAB_BEGIN;					/**< @brief The Interrupt Table, note that entries are addressed with their Priority, there are no fixed vectores.*/
...
_FARDATA const CoreInit_t CPUInit[3] = {
		...
		/* Core 1 */
		{
				...
				(uint32_t)__CSA_BEGIN,	// CSA base
				(uint32_t)__CSA_SIZE,	// CSA size
				&__INT_TAB_BEGIN,		// Initial Interrupt-Table
				&__INIT_TRAPTAB_BEGIN,	// The CPU is initialized with the init Trap-Table
