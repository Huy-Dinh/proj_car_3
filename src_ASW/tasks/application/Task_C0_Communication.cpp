...
#define TASK_C0_COMMUNICATION_INTR_STACKSIZE      200
...
static const PxProtectRegion_T Task_C0_Communication_Regions[] = {
	...
	//rte_time buffer for communication task lies in CPU0 data pool.
	{ (PxUInt_t) __CPU0_DATAPOOL_BEGIN, (PxUInt_t) __CPU0_DATAPOOL_END, (PxProtectType_t)WRProtection},

	//rte_time related variables lies in global task section.
	...
	//@PF - wild pointer? SOmebody tries to write on this data
...
/******************************* ISR for RTE Timing *************************************************/
//UART 0 USB
void UART_ASCLIN0_RxISR(PxArg_t arg)
{
...
//UART 2 TX - X103(15) RX - X103(16)

void UART_ASCLIN1_RxISR(PxArg_t arg)
{
...
void UART_ASCLIN1_TxISR(PxArg_t arg)
{
...
//UART 4 TX - X103(13) RX - X103(14)

void UART_ASCLIN2_RxISR(PxArg_t arg)
{
	//Get the handle to the task receiving the event
	PxTask_t taskId = PxTaskIdSet(arg);
	...
	RC_t result = UART_ReadData(uart4, &data);
...
void CTask_C0_Communication::Task_Func(PxTask_t myID, PxMbx_t myMailbox, PxEvents_t myActivationEvents)
{
	...
	//Illegal memory access, for testing purposes only
	//Task_C0_Communication_id.id = 2;
	//asm("debug");
	...
	//Signal that all devices are initialised if RT_initExecute was successfull
	while (1)
	{
		...
		{
			PxMsgMetadata_t metadata = PxMsgGetMetadata(EveMsg.msg);

			//Depending on the Metadata, get the correct data out
			switch (metadata)
			{
			case MSGMETADATACONTENT_REMOTE:
				REMOTE_data_t* remoteptr = (REMOTE_data_t*)PxMsgGetData(EveMsg.msg);
				REMOTE_data_t remotedata = (*remoteptr);
				REMOTE_write(&so_out_remote, remotedata);
				break;
			}

			//Release message again
			PxMsgRelease(EveMsg.msg);
		}
		...
		{
			REMOTE_set_raw(&so_in_remote, &UARTPROT_remoteRxBuffer);
		...
		// Call of REMOTE_run_readAndCheckProtocol
