...
static const PxProtectRegion_T Task_C2_TFT_Regions[] = {
	...
	//Todo: Must be removed-Workaround for OS Bug with QSPI port
	{ (PxUInt_t) 0xF0000000, (PxUInt_t) 0xFFFFFFFF, (PxProtectType_t)WRProtection},

	//Stack memory of the task
	{ (PxUInt_t) __CPU2_TASK_BEGIN, (PxUInt_t) __CPU2_TASK_END, (PxProtectType_t)WRProtection},
	...
	//Signal Layer Miscellaneous Signals
...
#pragma section ".Task_C2_TFT.data"

static volatile int TFT__counterTest;
...
void QSPI0_txISR(PxArg_t arg)
{
	return _QSPI_TX_ISR(0);
}

void QSPI0_ptISR(PxArg_t arg)
{
	return _QSPI_PHASE_TRANSITION_ISR(0);
}

void QSPI0_errorISR(PxArg_t arg)
{
	return _QSPI_ERROR_ISR(0);
}
