...
#define ENGINE_K_X 		1.0
#define ENGINE_K_Y 		1.0
#define ENGINE_K_PHI 	1.0
...
*/
		...
		//speed limit == 100% ==> simple scaling of speed value
		//speed limit == 50% ==> scaling of speed value and limitation to 50%
		if (maxSpeedParamater > (float)carState.speedLimit)
		{
			fr *= (float)carState.speedLimit / maxSpeedParamater;
			fl *= (float)carState.speedLimit / maxSpeedParamater;
			rl *= (float)carState.speedLimit / maxSpeedParamater;
			rr *= (float)carState.speedLimit / maxSpeedParamater;
		}
...
RT_safety_t ENGINE_run_setEngineSpeedCarState()
{
	...
	{
		//No need to stop the car. In Preoperational this will not work, in running it is not intended
		return SAFEOK;
	}
